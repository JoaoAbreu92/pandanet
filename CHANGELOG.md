...
🔒 WhatsPanda: Privacidade de Histórico & Isolamento de Setor
- **Isolamento de Conversas por Departamento**: Atendentes de um setor específico (ex: Suporte) não têm acesso ao histórico de mensagens trocadas com o mesmo contato de WhatsApp por outro setor (ex: Financeiro), garantindo sigilo absoluto de dados corporativos e financeiros confidenciais.
- **Rastreamento de Origem das Mensagens**: Todas as mensagens recebidas via webhook, enviadas por chatbot de triagem ou logs automáticos agora registram persistentemente o `queue_id` correspondente do atendimento.

🤖 WhatsPanda: Automação de Encerramento (Mensagem de Conclusão)
...
- **Fechamento Inteligente (keepClosed)**: O envio automático não reabre o ticket no painel, mantendo o status fechado de forma consistente.

⚡ WhatsPanda: Alta Resiliência de Sincronização & Remoção Imediata
- **Sincronização Ininterrupta (Short-Polling Fallback)**: Adicionado fallback de polling automatizado a cada 5 segundos para mensagens da conversa ativa e a cada 10 segundos para a lista de conversas, contornando oscilações de conexão em tempo real.
- **Remoção de Fila sem Latência**: Correção do delay ao aceitar atendimentos; agora os tickets mudam de status e saem da aba "Aguardando" em tempo real sem a necessidade de atualizar a página manualmente.

🎨 WhatsPanda: Polimento nos Controles de Configuração
...
📋 Portal Meu RH: Centralização de Solicitações & Novos Botões
- **Reorganização de Submenus**: Remoção do submenu duplicado de "Solicitações" abaixo do Portal Meu RH, centralizando e limpando o fluxo de navegação do menu lateral.
- **Novas Solicitações Integradas**: Adição de formulários dedicados para Solicitação de EPI, Solicitação de Uniformes e Solicitação de Documentos.
- **Sincronia Visual e Alinhamento**: Simplificação do texto de ação de todos os botões para ler apenas "Solicitar". Ajuste de alinhamento e assimetria para garantir visual uniforme, moderno e premium.
- **Ativação de Botão Faltante**: Restauração completa da funcionalidade do botão de "Alteração de Dados" na tela do portal.

🔧 Infraestrutura & Builds
- **Configuração de Build APK**: Configuração de JDK 21 (`org.gradle.java.home`) para compatibilidade e compilação limpa do APK do aplicativo mobile no Gradle local.
- **Script de Migração**: Criação do script de banco de dados (`20260703_add_whatspanda_close_message_and_privacy.sql`) para suporte nativo a campos de isolamento e envio de mensagens.
