                    ...
                    <div className="relative">
                        ...
                        {isDropdownOpen && (
                            <div className="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-lg py-1 z-50 dark:bg-gray-800 dark:border dark:border-gray-700">
                                ...
                                <a href="#" onClick={(e) => { e.preventDefault(); onNavigate('profile'); setDropdownOpen(false); }} className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700">
                                    <UserCircleIcon className="w-5 h-5 mr-2" /> {t('header.profile')}
                                </a>
                                {currentUser.isAdmin && !isImpersonating && (
                                    <a href="#" onClick={(e) => { e.preventDefault(); onNavigate('admin'); setDropdownOpen(false); }} className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700">
                                        <Cog6ToothIcon className="w-5 h-5 mr-2" /> {t('sidebar.admin')}
                                    </a>
                                )}
                                <a href="#" onClick={(e) => { e.preventDefault(); onLogout(); }} className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700">
