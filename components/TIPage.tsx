            ...
            <div className="space-y-3">
                {systems.map((system, index) => (
                    <div key={index} className="flex justify-between items-center p-2 rounded-md hover:bg-gray-50">
                        <span className="text-brand-text">{system.name}</span>
                        <div className="flex items-center space-x-2">
                            <span className="text-sm text-brand-subtle-text">{system.status}</span>
                            {getStatusIndicator(system.status)}
                        </div>
                    </div>
                ))}
            </div>
             <p className="text-xs text-gray-400 mt-4 text-right">Atualizado agora</p>
        </Card>
    );
};
...
const SecurityTips: React.FC = () => {
    ...
    return (
        <Card title="Dica de Segurança" headerAction={<ShieldCheckIcon className="w-6 h-6 text-gray-400" />}>
            <p className="text-brand-subtle-text italic">"{randomTip}"</p>
        </Card>
    )
}
            ...
            <div className="space-y-2">
                {faqs.map((faq, index) => (
                    <div key={index} className="border-b last:border-b-0">
                        <button onClick={() => setOpenFaq(openFaq === index ? null : index)} className="w-full flex justify-between items-center py-3 text-left font-semibold text-brand-text">
                            <span>{faq.q}</span>
                            <ChevronDownIcon className={`w-5 h-5 transition-transform ${openFaq === index ? 'rotate-180' : ''}`} />
                        </button>
                        {openFaq === index && (
                            <div className="pb-3 text-brand-subtle-text">
                                {faq.a}
                            </div>
                        )}
                    </div>
...
const TIPage: React.FC<TIPageProps> = ({ onNavigate }) => {
    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-3xl font-bold text-brand-text">Painel de TI</h1>
                <p className="mt-1 text-lg text-brand-subtle-text">Seu portal para recursos e suporte de tecnologia.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div onClick={() => onNavigate('tickets')} className="p-8 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer flex flex-col items-center text-center">
                    <Cog6ToothIcon className="w-12 h-12 text-brand-primary mb-4"/>
                    <h2 className="text-xl font-bold text-brand-text">Abrir um Chamado</h2>
                    <p className="text-brand-subtle-text mt-2">Precisa de ajuda? Abra um chamado para nossa equipe de suporte técnico.</p>
                </div>
                 <div onClick={() => onNavigate('ti-requests')} className="p-8 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer flex flex-col items-center text-center">
                    <PlusIcon className="w-12 h-12 text-brand-primary mb-4"/>
                    <h2 className="text-xl font-bold text-brand-text">Solicitar Equipamento</h2>
                    <p className="text-brand-subtle-text mt-2">Solicite um novo hardware ou software para suas atividades.</p>
                </div>
            </div>
